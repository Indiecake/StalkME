...
function getReceivedMessages(req, res) {
    ...
    Message.find({ receiver: loggedUser }).populate('emitter', 'name surname nick image _id').sort('-createdAt').paginate(page, itemsPerPage, (err, messages, total) => {
        if (err) return res.status(500).send({ message: 'Error en la peticion' });
...
function getEmittedMessages(req, res) {
    ...
    Message.find({ emitter: loggedUser }).populate('emitter receiver', 'name surname nick image _id').sort('-createdAt').paginate(page, itemsPerPage, (err, messages, total) => {
        if (err) return res.status(500).send({ message: 'Error en la peticion' });
